...
def clip_grad_norm_(parameters, grad_max_norm):
    ...
    torch.nn.utils.clip_grad_norm_(parameters, grad_max_norm)
    return total_norm
...
def get_args():
    ...
    parser.add_argument("--save-speed-key", type=str, default=None, help="Identifier for this run's speed (e.g., 'baseline' or 'flash')")
    parser.add_argument("--speed-results-path", type=str, default="speed_results.json")
    parser.add_argument(
        "--disable-pytorch-flash",
        action="store_true",
        help="Disable PyTorch’s Flash-2 (SDP kernel) for a second baseline run",
    )
    ...
    type=str,
    default=None,
    help="Path to baseline model to compare against (used in FlashAttention runs)."
    )
    ...
    parser.add_argument(
        "--use-flash-attn",
        action="store_true",
        help="Use FlashAttention instead of standard attention",
    )
